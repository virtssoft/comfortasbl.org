...
// Utilitaire pour transformer le titre en slug (ex: "Mon Article" -> "mon-article")
...
const Blog: React.FC = () => {
  ...
  // Mise à jour du titre de l'onglet pour le SEO
  useEffect(() => {
    document.title = `Blog | COMFORT Asbl - Récits de Vie`;
  }, []);
      ...
      <section className="py-32 border-b border-gray-100">
         <div className="container mx-auto px-6">
            <div className="flex flex-col items-center text-center max-w-3xl mx-auto">
                ...
                {/* Correction des clés de traduction selon ton fichier translations */}
                <h1 className="text-5xl md:text-7xl font-serif font-bold text-comfort-blue mb-8 tracking-tight">
        ...
        <div className="container mx-auto px-6">
            {currentPosts.length > 0 ? (
                        ...
                        <div key={post.id} className={`group ${i % 2 !== 0 ? 'md:translate-y-12' : ''} transition-all duration-700`}>
                            ...
                            <div className="space-y-6">
                                ...
                                <h3 className="text-2xl font-serif font-bold text-comfort-dark group-hover:text-comfort-blue transition-colors leading-tight">
                                    {/* Utilisation du slug dans l'URL */}
                                    <Link to={`/blog/${post.id}/${slug}`}>{post.title}</Link>
                                </h3>
                                <p className="text-gray-500 text-sm font-light leading-relaxed line-clamp-3 italic">
                                    "{post.excerpt}"
                                </p>
                                <Link to={`/blog/${post.id}/${slug}`} className="inline-flex items-center text-comfort-blue font-bold text-xs uppercase tracking-widest hover:text-comfort-gold transition-colors">
                                    {t('common.more')} <ArrowRight size={14} className="ml-3" />
                                </Link>
                            </div>
                        </div>
                    )})}
                ...
                <div className="text-center py-40">
                    ...
                    <p className="text-gray-400 font-serif italic text-xl">{t('common.not_found')}</p>
                </div>
            )}
        </div>
      </section>
