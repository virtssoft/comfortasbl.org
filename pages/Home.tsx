...
import { 
  ...
  ChevronLeft, ChevronRight, Eye, Download, Newspaper, 
  Quote, MessageSquare, Send, User, Briefcase, Phone, Mail, CheckCircle 
} from 'lucide-react';
...
const Home: React.FC = () => {
  ...
  // Calcul dynamique de l'ancienneté (ex: Fondation en 2012)
  const yearsOfExistence = new Date().getFullYear() - 2012;
  ...
  const handleTestimonySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success || res.message?.includes('succès')) {
          setShowSuccess(true);
          setTestimonyForm({ nom_complet: '', email: '', telephone: '', role: 'benevole', organisation: '', message: '' });
    ...
    <div className="flex flex-col min-h-screen bg-white font-sans overflow-x-hidden">
      ...
      <section className="relative h-[85vh] md:h-[90vh] bg-comfort-dark overflow-hidden">
        {heroItems.map((post, idx) => (
          ...
          >
            <div className="absolute inset-0">
               <img src={post.image} alt={post.title} className="w-full h-full object-cover scale-105 animate-ken-burns" />
            ...
            <div className="container relative z-20 mx-auto h-full flex items-center px-6 lg:px-12">
              <div className="max-w-2xl">
                ...
                <h1 className="text-3xl md:text-5xl lg:text-6xl font-serif font-bold text-white mb-6 leading-tight animate-fade-in-up">{post.title}</h1>
                <p className="text-lg md:text-xl text-gray-300 mb-10 font-light border-l-2 border-white/20 pl-6 line-clamp-3 animate-fade-in-up delay-200">{post.excerpt}</p>
                <div className="flex flex-col sm:flex-row gap-4 animate-fade-in-up delay-500">
                  <Link to={`/blog/${post.id}`} className="bg-comfort-gold text-white px-10 py-4 font-bold uppercase tracking-widest hover:bg-white hover:text-comfort-dark transition-all text-center text-xs shadow-xl">Découvrir l'histoire</Link>
                  <Link to="/donate" className="border border-white/30 text-white px-10 py-4 font-bold uppercase tracking-widest hover:bg-white/10 transition-all text-center text-xs backdrop-blur-sm">Soutenir l'action</Link>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="absolute bottom-12 right-12 z-30 flex items-center space-x-6">
            <button onClick={prevHero} className="text-white/50 hover:text-white border border-white/20 p-4 rounded-full transition-colors hover:bg-white/5"><ChevronLeft size={24} /></button>
            <button onClick={nextHero} className="text-white/50 hover:text-white border border-white/20 p-4 rounded-full transition-colors hover:bg-white/5"><ChevronRight size={24} /></button>
        </div>
      </section>
          ...
          <div className="grid lg:grid-cols-2 gap-24 items-center">
            <div className="relative">
              ...
              <h2 className="text-4xl md:text-6xl font-serif font-bold text-comfort-blue mb-8 leading-tight">
                L'intégrité au service de <span className="italic font-light text-comfort-gold">l'humanité</span>.
              </h2>
            ...
            <div className="grid grid-cols-2 gap-6">
               <div className="space-y-6 pt-12">
                  <img src="https://api.comfortasbl.org/assets/images/about-who.jpg" className="aspect-[3/4] object-cover rounded-sm shadow-2xl hover:grayscale transition-all duration-500" alt="Mission" />
                  <div className="p-8 bg-comfort-blue text-white rounded-sm shadow-xl transform -rotate-2">
                    <h4 className="text-5xl font-serif font-bold mb-2">{projects.length}+</h4>
                    <p className="text-[10px] uppercase tracking-widest font-bold opacity-80">Programmes actifs</p>
                  </div>
               </div>
               <div className="space-y-6">
                  <div className="p-8 bg-comfort-gold text-white rounded-sm shadow-xl transform rotate-2">
                  ...
                  <img src="https://api.comfortasbl.org/assets/images/about-hero.jpg" className="aspect-[3/4] object-cover rounded-sm shadow-2xl hover:grayscale transition-all duration-500" alt="Vision" />
               </div>
            </div>
          </div>
        </div>
      ...
      {/* 🏛️ DOMAINES STRATÉGIQUES */}
      <section className="py-32 bg-comfort-light border-y border-gray-100">
        <div className="container mx-auto px-6 text-center">
            <h2 className="text-4xl font-serif font-bold text-comfort-blue mb-20 relative inline-block">
              Domaines Stratégiques
              <span className="absolute -bottom-4 left-1/2 -translate-x-1/2 w-12 h-1 bg-comfort-gold"></span>
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-0 border border-gray-100 shadow-sm overflow-hidden">
              {DOMAINS.map((domain) => (
                 ...
                 {testimonials.length > 0 ? (
                    <div className="relative z-10 animate-fade-in">
                       ...
                       <div className="flex items-center space-x-6">
                          <div className="w-16 h-16 bg-comfort-blue rounded-full flex items-center justify-center text-white text-xl font-bold shadow-xl border-4 border-white">
                          ...
                          <div>
                             ...
                             <p className="text-xs text-comfort-gold font-bold uppercase tracking-widest mt-1">
                    ...
                    <p className="text-center text-gray-400 italic">Chargement des témoignages...</p>
                 )}
           ...
           <div className="max-w-4xl mx-auto bg-comfort-dark p-12 md:p-16 rounded-sm shadow-2xl relative overflow-hidden">
              {showSuccess ? (
                <div className="relative z-10 text-center py-12 animate-fade-in">
                  <CheckCircle size={64} className="text-comfort-gold mx-auto mb-6" />
                  <h3 className="text-2xl text-white font-serif mb-2">Message envoyé !</h3>
                  <p className="text-gray-400">Merci de partager votre expérience. Votre témoignage sera publié après modération.</p>
                </div>
              ) : (
                <>
      ...
      <section className="py-32 bg-comfort-light">
        <div className="container mx-auto px-6">
          <div className="flex items-center justify-between mb-20">
              <h2 className="text-4xl font-serif font-bold text-comfort-blue">Dernières Publications</h2>
            ...
            {bulletins.slice(0, 2).map((bulletin) => (
              <div key={bulletin.id} className="bg-white p-12 border border-gray-100 flex flex-col md:flex-row items-center gap-10 hover:shadow-2xl transition-all group">
                 ...
                 <div className="flex-1">
                    <span className="text-[10px] font-bold text-comfort-gold uppercase tracking-widest block mb-2">Rapport Officiel</span>
                    <h3 className="text-xl font-serif font-bold text-comfort-blue mb-4 leading-tight">{bulletin.title}</h3>
                    <a href={getFullPdfUrl(bulletin.pdf_path)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center text-xs font-bold uppercase tracking-widest text-comfort-blue hover:text-comfort-gold transition-colors">
          ...
          <div className="grid md:grid-cols-3 gap-12">
            {blogPosts.slice(0, 3).map((post) => (
              <div key={post.id} className="group bg-white rounded-sm overflow-hidden shadow-sm hover:shadow-xl transition-all">
                 ...
                 <div className="p-8">
                    <h3 className="text-xl font-serif font-bold text-comfort-dark mb-4 group-hover:text-comfort-blue transition-colors">
